...
Private hWndRTB(0 To 3) As Long
Private hWndParent As Long

Public Sub EnableURLDetect(ByVal hWndRichTextbox As Long, ByVal hWndOwner As Long, ByVal Console As eConsoleType)
'***************************************************
'Author: ZaMa
...
On Error GoTo ErrHandler
    ...
    hWndRTB(Console) = hWndRichTextbox
  
  Exit Sub
  
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub EnableURLDetect de modConsole.bas")
End Sub
...
On Error GoTo ErrHandler
    ...
    Dim I As Integer
    For I = 0 To 3
        SendMessage hWndRTB(I), EM_AUTOURLDETECT, 0, ByVal 0
    Next I
    StopCheckingLinks
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub DisableURLDetect de modConsole.bas")
End Sub
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub StartCheckingLinks de modConsole.bas")
End Sub
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub StopCheckingLinks de modConsole.bas")
End Sub
...
On Error GoTo ErrHandler
    ...
    If uMsg = WM_NOTIFY Then
        ...
        If (uHead.hWndFrom = hWndRTB(eConsoleType.General)) And (uHead.code = EN_LINK) Or _
            (uHead.hWndFrom = hWndRTB(eConsoleType.Agrupaciones)) And (uHead.code = EN_LINK) Or _
                (uHead.hWndFrom = hWndRTB(eConsoleType.Acciones)) And (uHead.code = EN_LINK) Or _
                    (uHead.hWndFrom = hWndRTB(eConsoleType.Custom)) And (uHead.code = EN_LINK) Then
            ...
            Select Case eLink.msg
                Case WM_LBUTTONDOWN
                    ...
                    If frmMain.RecTxt(eConsoleType.General).Visible Then _
                        lLen = SendMessage(hWndRTB(eConsoleType.General), EM_GETTEXTRANGE, 0, eText)
                    If frmMain.RecTxt(eConsoleType.Agrupaciones).Visible Then _
                        lLen = SendMessage(hWndRTB(eConsoleType.Agrupaciones), EM_GETTEXTRANGE, 0, eText)
                    If frmMain.RecTxt(eConsoleType.Acciones).Visible Then _
                        lLen = SendMessage(hWndRTB(eConsoleType.Acciones), EM_GETTEXTRANGE, 0, eText)
                    If frmMain.RecTxt(eConsoleType.Custom).Visible Then _
                        lLen = SendMessage(hWndRTB(eConsoleType.Custom), EM_GETTEXTRANGE, 0, eText)
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Function WndProc de modConsole.bas")
End Function
