...
Begin VB.Form frmBancoAcc 
   ...
   Begin ARGENTUM.AOPictureBox PicInv 
      ...
      _ExtentY        =   5900
   End
   Begin VB.Image imgGoToCharacterBank 
      Height          =   525
      Left            =   5400
      Top             =   270
      Width           =   1230
...
Private cButtonChangePassword As clsGraphicalButton
Private cButtonWhitdrawItem As clsGraphicalButton
Private cButtonDepositItem As clsGraphicalButton

Private cButtonGoToCharacterBank As clsGraphicalButton
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub dragBankInventory_dragDone de frmBancoAcc.frm")
End Sub
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub dragBankInventory_updateSlotInfo de frmBancoAcc.frm")
End Sub
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub dragUserInventory_dragDone de frmBancoAcc.frm")
End Sub
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub dragUserInventory_updateSlotInfo de frmBancoAcc.frm")
End Sub
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub cantidad_Change de frmBancoAcc.frm")
End Sub
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub cantidad_KeyPress de frmBancoAcc.frm")
End Sub
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub CantidadOro_Change de frmBancoAcc.frm")
End Sub
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub CantidadOro_KeyPress de frmBancoAcc.frm")
End Sub
...
On Error GoTo ErrHandler
    ...
    ' Inventories.
    Set dragBankInventory = AccBank(1)
    Set dragUserInventory = AccBank(0)
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub Form_Load de frmBancoAcc.frm")
End Sub
...
On Error GoTo ErrHandler
    ...
    Call cButtonGoToCharacterBank.Initialize(imgGoToCharacterBank, GrhPath & "BotonPersonajeFlechaIzquierda.jpg", GrhPath & "BotonPersonajeFlechaIzquierda.jpg", GrhPath & "BotonPersonajeFlechaIzquierda.jpg", Me)
    ...
    Call cButtonChangePassword.Initialize(imgCambiar, GrhPath & "BotonLlaveAmarilla.jpg", GrhPath & "BotonLlaveAmarilla.jpg", GrhPath & "BotonLlaveAmarilla.jpg", Me)
   
  Exit Sub
  
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub LoadButtons de frmBancoAcc.frm")
End Sub
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub Form_Unload de frmBancoAcc.frm")
End Sub

Private Sub imgCambiarContraseña_Click()
    frmBancoAccChangePass.Show , Me
End Sub
...
On Error GoTo ErrHandler
    ...
    If AccBank(Index).SelectedItem = 0 Then Exit Sub
    
    If Not IsNumeric(cantidad.text) Then Exit Sub
    
    Select Case Index
        Case 0
            LastIndex1 = AccBank(0).SelectedItem
            LasActionBuy = True
            Call WriteAccBankExtractItem(AccBank(0).SelectedItem, cantidad.text)
            
       Case 1
            LastIndex2 = AccBank(1).SelectedItem
            LasActionBuy = False
            Call WriteAccBankDeposit(AccBank(1).SelectedItem, cantidad.text)
    End Select
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub imgFlecha_Click de frmBancoAcc.frm")
End Sub

Private Sub imgDepositarOro_Click()
    Call WriteAccBankDepositGold(Val(CantidadOro.text))
End Sub
...
On Error GoTo ErrHandler
    ...
    If AccBank(1).SelectedItem = 0 Then Exit Sub
    ...
    Exit Sub
ErrHandler:
    Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub imgDepositItem_Click de frmBancoAcc.frm")
End Sub

Private Sub imgGoToCharacterBank_Click()
    Call CerrarVentana
    Call WriteBankStart
End Sub

Private Sub imgRetirarOro_Click()
    Call WriteAccBankExtractGold(Val(CantidadOro.text))
End Sub
...
On Error GoTo ErrHandler
    ...
    If AccBank(0).SelectedItem = 0 Then Exit Sub
    ...
    Exit Sub
ErrHandler:
    Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub imgWhitdrawItem_Click de frmBancoAcc.frm")
End Sub
...
On Error GoTo ErrHandler
    ...
    If AccBank(0).SelectedItem <= 0 Then
        Label1(0).Caption = ""
    ...
    With AccBankInventory(AccBank(0).SelectedItem)
        ...
        Select Case .OBJType
            ...
            Case 3, 16, 17
                ItemDescription = "Defensa Mínima: " & .MinDef & vbCrLf & "Defensa Máxima: " & .MaxDef
                ShowItemLevel = True
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub PicBancoInv_Click de frmBancoAcc.frm")
End Sub
...
On Error GoTo ErrHandler
    ...
    If AccBank(1).SelectedItem <= 0 Or AccBank(1).SelectedItem > AccBank(1).MaxObjs Then
        Label1(0).Caption = ""
        Label1(1).Visible = False
        Exit Sub
    End If
    ...
    With Inventario
        Label1(0).Caption = .ItemName(AccBank(1).SelectedItem)
        Label1(0).Visible = True
        
        ItemMinLevel = GameMetadata.Objs(.ObjIndex(AccBank(1).SelectedItem)).MinimumLevel
        
        
        Select Case .OBJType(AccBank(1).SelectedItem)
            Case eObjType.otWeapon, eObjType.otFlechas, eObjType.otTool
                ItemDescription = "Golpe Mínimo: " & .MinHit(AccBank(1).SelectedItem) & vbCrLf & "Golpe Máximo: " & .MaxHit(AccBank(1).SelectedItem)
                ShowItemLevel = True
                Label1(1).Visible = True
                
            Case eObjType.otCasco, eObjType.otArmadura, eObjType.otEscudo ' 3, 16, 17
                ItemDescription = "Defensa Mínima: " & .MinDef(AccBank(1).SelectedItem) & vbCrLf & "Defensa Máxima: " & .MaxDef(AccBank(1).SelectedItem)
                ShowItemLevel = True
                Label1(1).Visible = True
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub PicInv_Click de frmBancoAcc.frm")
End Sub
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub imgCerrar_Click de frmBancoAcc.frm")
End Sub
...
On Error GoTo ErrHandler
    ...
    Call WriteAccBankEnd
    NoPuedeMover = False
  
  Exit Sub
  
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub CerrarVentana de frmBancoAcc.frm")
End Sub
