...
Public Const GMCommand As String = "GMSG,SHOWNAME,ONLINECAOS,IRCERCA,REM,HORA,DONDE,NENE,TELEPLOC," & _
    "TELEP,SILENCIAR,SHOW,INVISIBLE,PANELGM,CARCEL,ADVERTENCIA,MOD,INFO,STAT,BAL,INV,BOV,SKILLS," & _
    "REVIVIR,ONLINEGM,ONLINEMAP,PERDON,ECHAR,EJECUTAR,BAN,UNBAN,SEGUIR,SUM,CC,RESETINV,RMSG,MAPMSG," & _
    "NICK2IP,ONCLAN,CT,DT,LLUVIA,SETDESC,FORCEMIDIMAP,FORCEWAVMAP,REALMSG,CAOSMSG,CIUMSG,TALKAS,MASSDEST," & _
    "PISO,ESTUPIDO,KICKCONSE,TRIGGER,BANIPLIST,BANIPRELOAD,DUMPSECURITY,MIEMBROSCLAN,BANCLAN,UNBANIP," & _
    "CI,DEST,ECHARFACCION,FORCEMIDI,FORCEWAV,BLOQ,MATA,MASSKILL,SMSG,ACC,RACC,AI,AC,NAVE,HABILITAR," & _
    "APAGAR,CONDEN,RAJAR,ECHARCLAN,LASTEMAIL,APASS,AEMAIL,ANAME,DOBACKUP,SHOWCMSG,GUARDAMAPA,MODMAPINFO," & _
    "GRABAR,BORRAR,ECHARTODOSPJ,RELOADNPCS,RELOADSINI,RELOADHECHIZOS,RELOADOBJ,REINICIAR,AUTOUPDATE," & _
    "CHATCOLOR,IGNORADO,SLOT,SETINIVAR,CREARPRETORIANOS,ELIMINARPRETORIANOS,DENUNCIAS,SETDIALOG,INFOJEFES"
    
Public ListGMCommand() As String
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub AuxWriteWhisper de ProtocolCmdParse.bas")
End Sub
...
On Error GoTo ErrHandler
    ...
    If Left$(Comando, 1) = "/" Then
        ...
        If Not esGM(UserCharIndex) And IsGMCommand(Comando) Then
            Exit Sub
        End If
        ...
        Select Case Comando
            ...
            Case "/ACEPTARDUELO"
                Call WriteAceptarDuelo
            
            Case "/RECHAZARDUELO"
                Call WriteRechazarDuelo
                
            Case "/CANCELARDUELO"
                Call WriteCancelarDuelo
            ...
            Case "/APOSTAR"
                ...
                If notNullArguments Then
                    If ValidNumber(ArgumentosRaw, eNumber_Types.ent_Integer) Then
                        Call WriteGamble(CInt(ArgumentosRaw))
                    Else
                        'No es numerico
            ...
            Case "/INVITAR"
                If notNullArguments Then
                    Call WritePartyInviteMember(ArgumentosRaw)
                Else
                    'Avisar que falta el parametro
                    Call ShowConsoleMsg("Faltan parámetros. Utilice /invitar NICKNAME.")
                End If
            ...
            'TODO_TORNEO
            Case "/PARTICIPAR"
                Call WriteTournamentParticipate
 
            Case "/DESCALIFICAR"
                If notNullArguments Then
                    Call WriteTournamentDisqualify(ArgumentosRaw)
                Else
                    Call ShowConsoleMsg("Faltan parámetros. Utilice /DESCALIFICAR <NICK> .")
                End If
...
'                        Call WriteTournamentFight(tmpArr(0), tmpArr(1))
'                    Else
'                        Call ShowConsoleMsg("Faltan parámetros. Utilice /PELEA <Participante1> @ <Participante2>.")
'                    End If
'                Else
'                    Call ShowConsoleMsg("Faltan parámetros. Utilice /PELEA <Participante1> @ <Participante2>.")
'                End If
            ...
            Case "/CERRARTORNEO"
                Call WriteTorunamentCancel
 
'            Case "/HTORNEO"
'                If notNullArguments Then
'                    tmpArr = Split(ArgumentosRaw, "@")
'                    If UBound(tmpArr) = 1 Then
'                        If ValidNumber(tmpArr(0), eNumber_Types.ent_Byte) And ValidNumber(tmpArr(1), eNumber_Types.ent_Byte) Then
'                            Call WriteTorunamentBegin(CByte(tmpArr(0)), CByte(tmpArr(1)))
'                        Else
'                            'No es numerico
'                            Call ShowConsoleMsg("Cupos y nivel incorrecto. Utilice /HTORNEO CUPOS@NIVEL.")
'                        End If
'                    Else
'                        Call ShowConsoleMsg("Faltan parámetros. Utilice /HTORNEO CUPOS@NIVEL.")
'                    End If
'                End If
            '/TODO_TORNEO
            ...
            Case "/PERDON"
                ...
                Else
                    Call ShowConsoleMsg("Escriba el nombre del personaje a perdonar")
                End If
            
            Case "/SPAWNBOSS"
                If notNullArguments Then
                    Call WriteSpawnBoss(ArgumentosRaw)
                Else
                    'Avisar que falta el parametro
                    Call ShowConsoleMsg("Escriba el número del boss a invocar.")
                End If
            ...
            Case "/NENE"
                ...
                Else
                    ...
                    Call WriteCreaturesInMap(PlayerData.CurrentMap.Number)
                End If
            ...
            Case "/TELEP"
                ...
                ElseIf CantidadArgumentos = 3 Then
                    ...
                    ElseIf ValidNumber(ArgumentosAll(1), eNumber_Types.ent_Byte) And ValidNumber(ArgumentosAll(2), eNumber_Types.ent_Byte) Then
                        ...
                        Call WriteWarpChar(ArgumentosAll(0), PlayerData.CurrentMap.Number, ArgumentosAll(1), ArgumentosAll(2))
                    Else
                ...
                ElseIf CantidadArgumentos = 2 Then
                    If ValidNumber(ArgumentosAll(0), eNumber_Types.ent_Byte) And ValidNumber(ArgumentosAll(1), eNumber_Types.ent_Byte) Then
                        ...
                        Call WriteWarpChar("YO", PlayerData.CurrentMap.Number, ArgumentosAll(0), ArgumentosAll(1))
                    Else
                        'No uso ningun formato por defecto
                        Call ShowConsoleMsg("Valor incorrecto. Utilice /telep NICKNAME MAPA X Y.")
                    End If
            ...
            Case "/CARCEL"
                If notNullArguments Then
                    ...
                    Call WriteGetPunishmentTypeList(ArgumentosRaw, ePunishmentSubType.Jail)
                    'Call WriteJail(tmpArr(0), "", 25)
            ...
            Case "/ADVERTENCIA"
                If notNullArguments Then
                    ...
                    Call WriteGetPunishmentTypeList(ArgumentosRaw, ePunishmentSubType.Warning)
            ...
            Case "/MOD"
                If notNullArguments And CantidadArgumentos >= 3 Then
                    Select Case UCase$(ArgumentosAll(1))
                        ...
                        Case "PP"
                            tmpInt = eEditOptions.eo_PlayerPoints
            ...
            Case "/BAN"
                If notNullArguments Then
                    Call WriteGetPunishmentTypeList(ArgumentosRaw, ePunishmentSubType.Ban)
                Else
                    'Avisar que falta el parametro
                    Call ShowConsoleMsg("Faltan parámetros. Utilice /ban NICKNAME.")
                End If
            ...
            Case "/ECHARFACCION"
                If notNullArguments Then
                    Call WriteFactionKick(ArgumentosAll(0))
                Else
                    Call ShowConsoleMsg("Faltan parámetros. Utilice /ECHARCLAN NOMBRE@MOTIVO.")
                End If
            ...
            Case "/MODCLANCONTRI"
                If notNullArguments = False Then
                    Call ShowConsoleMsg("Comando incorrecto. Utilice /MODCLANCONTRI NombreClan@cantidad")
                    Exit Sub
                End If
                ...
                If Not ValidNumber(tmpArr(1), eNumber_Types.ent_Integer) Then
                    Call ShowConsoleMsg("Comando incorrecto. Utilice /MODCLANCONTRI NombreClan@cantidad")
                    Exit Sub
                End If
                
                Call WriteModGuildContribution(tmpArr(0), tmpArr(1))
            ...
            Case "/MODMAPINFO" ' PK, BACKUP
                If CantidadArgumentos > 1 Then
                    Select Case UCase$(ArgumentosAll(0))
                        Case "INMOSINEFECTO"
                            Call WriteChangeMapInfoNoInmo(ArgumentosAll(1) = "1")
                            
                        Case "MISMOBANDO"
                            Call WriteChangeMapInfoMismoBando(ArgumentosAll(1) = "1")
            ...
            Case "/ACLAN"
                If notNullArguments Then
                    ...
                    Else
                        'Faltan los parametros con el formato propio
                        Call ShowConsoleMsg("Formato incorrecto. Utilice /CLAN ORIGEN@DESTINO.")
                    End If
                Else
                    'Avisar que falta el parametro
            ...
            Case "/ALIGNCLAN"
                If notNullArguments Then
                    ...
                    If UBound(tmpArr) = 1 Then
                        ...
                        If Not IsNumeric(tmpArr(1)) Then
                            Call ShowConsoleMsg("Formato incorrecto. Utilice /ALIGNCLAN NOMBRE CLAN@ALINEACION. Donde ALINEACION es un número entre 1 y 6.")
                        Else
                            Call WriteAdminChangeGuildAlign(tmpArr(0), tmpArr(1))
                        End If
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub ParseUserCommand de ProtocolCmdParse.bas")
End Sub
...
Public Sub ShowConsoleMsg(ByVal Message As String, Optional ByVal red As Integer = 255, _
    Optional ByVal green As Integer = 255, Optional ByVal blue As Integer = 255, _
    Optional ByVal bold As Boolean = False, Optional ByVal italic As Boolean = False, _
    Optional ByVal eeMessageType As eMessageType = eMessageType.Info)
'***************************************************
...
On Error GoTo ErrHandler
    ...
    Call AddtoRichTextBox(frmMain.RecTxt(0), Message, red, green, blue, bold, italic, , eeMessageType)
  
  Exit Sub
  
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub ShowConsoleMsg de ProtocolCmdParse.bas")
End Sub
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Function ValidNumber de ProtocolCmdParse.bas")
End Function
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Function validipv4str de ProtocolCmdParse.bas")
End Function
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Function str2ipv4l de ProtocolCmdParse.bas")
End Function
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Function AEMAILSplit de ProtocolCmdParse.bas")
End Function

Public Function IsGMCommand(ByVal Command As String) As Boolean

    Dim tempstr As String
    Dim I As Integer
    
    For I = 1 To UBound(ListGMCommand)
        tempstr = "/" & ListGMCommand(I)
        If tempstr = Command Then
            IsGMCommand = True
            Exit Function
        End If
    Next I
...
Public Sub LoadGMCommand()

    ListGMCommand = Split(GMCommand, ",")
