...
Public Sub SetResolution(ByVal changeResolution As Boolean)
'***************************************************
'Autor: Unknown
'Last Modification: 03/29/12
...
' 03/29/2012: D'Artagnan - New messagebox for changing the resolution.
'***************************************************
On Error GoTo ErrHandler
    ...
    'changeRes = SHMessageBoxCheck(frmCargando.hwnd, "¿Desea jugar en pantalla completa (800 x 600)?", "Argentum Online", vbYesNo, 0, "AORes")
    
    
    'If changeRes = 6 Then
    If changeResolution Then
        GameConfig.Graphics.bUseFullScreen = True
        NoRes = False
    'ElseIf changeRes = 7 Then
    Else
        GameConfig.Graphics.bUseFullScreen = False
        NoRes = True
    End If
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub SetResolution de Resolution.bas")
End Sub
...
ErrHandler:
  Call LogError("Error" & Err.Number & "(" & Err.Description & ") en Sub ResetResolution de Resolution.bas")
End Sub
